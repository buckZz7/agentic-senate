...
def test_fixtures_load_and_validate():
    ...
    assert len(fixtures) >= 6, "founding set has 6 fixtures"
    ids = [f["id"] for f in fixtures]
    assert len(ids) == len(set(ids)), "fixture ids must be unique"
    cats = {f["category"] for f in fixtures}
    assert {"genuinely_good", "subtle_bug", "dishonest_description",
            "privilege_escalation", "incentive_gaming",
            "honest_but_broken"} <= cats
